...
if($_SERVER["REQUEST_METHOD"]=="POST"){
	...
	if($Fact == 1){
	  ...
	  <input type="hidden" name="mensaje" value="Este pedido ya ha sido facturado">
      </form>
      </body>  ';
	...
	//go back 	
      ...
      <body onload="setTimeout(function() { document.frm1.submit() }, 500)">
      <form method="post" action="../cargafacturas.php" name="frm1"> 
	  <input type="hidden" name="mensaje" value="Pedido no encontrado">
      </form>
      </body>  ';
		...
		if(($result)==1){
			...
			/*$sql = "SELECT SaldoCredito FROM estadocuenta where IdEmpresa = '".$_POST["idempresa"]."'";
					...
					$uno = $row['SaldoCredito'];
					  ...
					  UPDateSaldoCredito($NvoSaldo, $IdEmpresa);
			
					}
					
					//set saldo comprometido 0 
					...
					$QueryCurrentS ="Select SaldoComprometido from estadocuenta where idempresa = '".$IdEmpresa."'";
					$resultC = $connect->query($QueryCurrentS); 
					if ($resultC->num_rows > 0) {
					while($rowC = $resultC->fetch_assoc()) {  
					$CurrentS = $rowC['SaldoComprometido'];
					  } 
					}
					 
					//get current saldo from estado cuenta 
					$NvoSaldoC = $CurrentS - $RSaldo; 
					
					//Update Nuevo SAldo 
					$updateNvoSaldo = "Update EstadoCuenta set SaldoComprometido = '".$NvoSaldoC."' where idempresa = '".$IdEmpresa."'";
					$resultNS = $connect->query($updateNvoSaldo); 
					*/
					
					
					
					//set saldo comprometido =
					...
					$Sobrante = $rowr['Sobrante'];
					  ...
					  if($Sobrante > 0){
						...
						$QInsert = "Insert into cargosaplicaciones (Factura, Importe, FP, Moneda, FechaV, TipoPago, TadBanco, Documento, Fecha, Abono, Intereses, IVA, IdEmpresa) 
							values ('".$Factura."','".$Total."','CONTADO','NACIONAL','".$Fecha."','PAGO CON RECIBO DE FONDOS','-','SALDO CONTADO','".$Fecha."', '".$Total."','0', '0', '".$IdEmpresa."')";
...
window.location.href='../cargafacturas.php';
	...
	mysqli_close($connect); 
      ...
      <body onload="setTimeout(function() { document.frm1.submit() }, 500)">
      <form method="post" action="../cargafacturas.php" name="frm1"> 
	  <input type="hidden" name="mensaje" value="Verifique el Folio Ingresado">
      </form>
      </body>  ';

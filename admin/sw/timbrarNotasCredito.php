...
$receptor["Nombre"] = $_POST['rzonsocialreceptor'];
...
for($i=0; $i<$productos; $i++){
    ...
    $traslado[0]["Base"] = $_POST['baseimpuestos'][$i]; //subtotal 
    ...
    $traslado[0]["Importe"] = $_POST['timporte'][$i];
...
try{
    ...
    if($resultadoJson->status=="success"){
        ...
        $ruta=$basePath.$resultadoJson->data->uuid.".xml";
            ...
            $ruta3=$basePath.'F'.str_pad($nvofolio, 10, '0', STR_PAD_LEFT).' '.$_POST['nombre'].' F-'.$_POST['foliopemex'].'.xml';
            $ruta4='F'.str_pad($nvofolio, 10, '0', STR_PAD_LEFT).' '.$_POST['nombre'].' F-'.$_POST['foliopemex'].'.xml';
        file_put_contents($ruta3, $resultadoJson->data->cfdi);
        
		// echo $resultadoJson->data->cfdi;
        //save QRCode
        $nombreyRuta = 'F'.str_pad($nvofolio, 10, '0', STR_PAD_LEFT).' '.$_POST['nombre'].' F-'.$_POST['foliopemex'].".png";

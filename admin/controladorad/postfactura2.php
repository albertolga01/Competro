...
global $i;
    ...
    function Credito()
    {
		...
		if ($ResPedido->num_rows > 0) {
			...
			$query = "Insert into facturas (Folio, IdEmpresa, Factura, Fecha, Total, pdf, NoPedido, 
                Descuento, Cantidad, ValorUnitario, RFC, flete, Concepto, FechaVencimiento, Restante, 
                Importe, ImpuestosTraslados, ImpuestosRetencion) 
		        values ('".$Folio."', '".$IdEmpresa."','".$Factura."', '".$Fecha."', '".$Total."', '
                ".$pdf."', '".$NoPedido."', '".$Descuento."', '".$Cantidad."', '".$ValorUnitario."', '
                ".$Rfc."', '".$flete."', '".$Concepto."', '".$FechaV."', '".$Total."', '".$Importee."', '
                ".$ImpuestosT."', '".$ImpuestosR."')"; 
				...
				if ($resulttr->num_rows > 0) {
					...
					if($Sobrante > 0){
						if($Sobrante > $Total){
					        ...
					        //Cargos y aplicaciones   
						    $QInsert = "Insert into cargosaplicaciones (Factura, Importe, FP, Moneda, FechaV, 
                                TipoPago, TadBanco, Documento, Fecha, Abono, Intereses, IVA, IdEmpresa) 
						        values ('".$Factura."','".$Total."','CONTADO','NACIONAL','".$Fecha."',
                                'PAGO CON RECIBO DE FONDOS','-','SALDO CONTADO','".$Fecha."', '".$Sobrante."','0', '0', '".$IdEmpresa."')";
    ...
    { 
		...
		if ($ResPedido->num_rows > 0) {
			...
			$FechaV =  date('Y-m-d', strtotime($Fecha. ' + '.$Plazo.' days'));  
	        $query = "Insert into facturas (Folio, IdEmpresa, Factura, Fecha, Total, pdf, NoPedido, Descuento, Cantidad, 
                ValorUnitario, RFC, flete, Concepto, FechaVencimiento, Restante, Importe, ImpuestosTraslados, ImpuestosRetencion) 
		        values ('".$Folio."', '".$IdEmpresa."','".$Factura."', '".$Fecha."', '".$Total."', '".$pdf."', '".$NoPedido."', '
                ".$Descuento."', '".$Cantidad."', '".$ValorUnitario."', '".$Rfc."', '".$flete."', '".$Concepto."', '".$FechaV."', '
                ".$Total."', '".$Importee."', '".$ImpuestosT."', '".$ImpuestosR."')"; 
			...
			$resulx = $connect->query($SQX);  
				...
				$Sqlupdatefacturado = "Update pedido set estadoatencion = 'FACTURADO', facturado = '1', programado = '0' 
                    where folio = '".$NoPedido."'";
                ...
                while($Importe>0){
					...
					if(isset($Disponible)){
						...
						if($Disponible >= $Importe){
							...
							$QInsert = "Insert into cargosaplicaciones (Factura, Importe, FP, Moneda, FechaV, TipoPago, 
                                TadBanco, Documento, Fecha, Abono, Intereses, IVA, IdEmpresa) 
							    values ('".$Factura."','".$Total."','CONTADO','NACIONAL','".$Fecha."','PAGO CON RECIBO DE FONDOS',
                                '-','".$Pago."','".$FF."', '".$Disponible."','0', '0', '".$IdEmpresa."')";
    ...
    function EnviarCorreo($Folio, $Correo, $Nombre, $Fecha, $Total, $pdf, $Factura)
    {
        ...
        try { 
            ...
            $mail->addAddress($Correo, 'Comercializadora');     
                // Add a recipient

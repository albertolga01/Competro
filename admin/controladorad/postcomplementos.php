...
use PHPMailer\PHPMailer\Exception; 
	...
	function Login() {	
        ...
        $NombreComplemento = $_POST["nombrecomplemento"];
		$query = "INSERT INTO complementos (IdEmpresa, Folio, FechaPago, UUIDfactura, NombreComplemento) 
                VALUES ('".$IdEmpresa."','".$Folio."','".$FechaPago."','".$UUIDfactura."','".$NombreComplemento."')";

		$result = mysqli_query($connect,$query);
		if ($result==1) {	
			$queryupdate = "UPDATE cargosaplicaciones SET generado='1' WHERE folio='".$_POST['foliocargo']."'";
			$resultupdate = mysqli_query($connect,$queryupdate);
			if($resultupdate==1){
				   $SQX = "Select correo, rzonsocial, usuario from empresa where idempresa = '".$IdEmpresa."'";
			...
			if ($resulx->num_rows > 0) {
			   ...
			   //
				...
				$Factura =  substr($NombreComplemento, 0,  -3);
				$xml = $Factura."xml";
				EnviarCorreo($Folio, $Correo, $Nombre, $FechaPago, $Importe, $NombreComplemento, $xml);

				 
				
				echo '
					<body onload="setTimeout(function() { document.frm1.submit() }, 500)">
						<form method="post" action="../pagosaplicacionesadmin.php" name="frm1">
							<input type="hidden" name="mensaje" value="Complemento Agregado Correctamente"/> 
						</form>
					</body> 
				';	
			} else { 
				echo '
					<body onload="setTimeout(function() { document.frm1.submit() }, 500)">
						<form method="post" action="../pagosaplicacionesadmin.php" name="frm1">
							<input type="hidden" name="mensaje" value="Error al actualizar cargo"/> 
						</form>
					</body> 
				';	
			}
			...
			echo '
				<body onload="setTimeout(function() { document.frm1.submit() }, 500)">
				    <form method="post" action="../pagosaplicacionesadmin.php" name="frm1">
				        <input type="hidden" name="mensaje" value="Error al Agregar Complemento"/> 
				    </form>
				</body> 
			';	
			exit();
...
<div style="text-align: left;"><strong>Serie y Folio: C '.$Folio.'<br>
Fecha de emisión: '.$Fecha.'</strong><br>
<strong>Tipo de comprobante:</strong>&nbsp;Complemento de Pago<br>
<strong>Importe: $'.number_format($Total, 2, '.', ',').'</strong></div>

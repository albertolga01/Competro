<?php
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST"){
		...
		//   
	    echo '
		 <body onload="setTimeout(function() { document.frm1.submit() }, 8000)">
		    <form method="post" action="../reporte_facturas_admin.php" name="frm1"> 
		    <input type="hidden" name="mensaje" value="Nota de crédito agregada correctamente: '.$Folio.'">
	...
	function uno(){	
		...
		$query = "Insert into notascredito (Folio, NotaCredito, IdEmpresa, Rfc, Pdf, UUIDfactura, Fecha, Importe, Total, Empresa, Referencia, Concepto, ValorUnitario, Cantidad) 
			values ('".$Folio."', '".$NotaCredito."','".$IdEmpresa."', '".$Rfc."', '".$pdf."', '".$UUIDfactura."', '".$Fecha."',
			'".$Importee."', '".$Total."', '".$Empresa."', '".$Referencia."', '".$Concepto."', '".$ValorUnitario."', '".$Cantidad."')"; 
			//echo $query;
		$result = mysqli_query($connect,$query); 
		 
		if(($result)==1){ 
	        echo "<script>
                alert('Nota de Credito Agregada Correctamente: ".$Folio."'); 
                </script>
            ";


            
							//folio movimiento / pago
							 $Qr = "select folio from movimientos order by folio desc limit 1";
							 $Rrx = $connect->query($Qr);
						 

								if ($Rrx->num_rows > 0) {
									while($rowr = $Rrx->fetch_assoc()) {  
									$foliopago = $rowr["folio"];
							     	}
			...
			*/
			...
			echo "<script>
			    alert('Error al Agregar Nota de Credito Folio: ".$Folio." Ya Existe');
			    </script>
            ";
		}
	}
    ...
    function Credito(){		  
        ...
        while($contador<20){
			...
			$W = "Select folio, factura, fecha, restante, total from facturas where restante > 0 and idempresa = '".$IdEmpresa."' order by fecha asc limit 1";
			...
			if ($resultt->num_rows > 0) {
                ...
                if($a >= $restante){ 				
                    ...
                    $QInsert = "Insert into cargosaplicaciones (foliopago, Factura, Importe, FP, Moneda, FechaV, TipoPago, TadBanco, Documento, Fecha, Abono, Intereses, IVA, IdEmpresa) 
                                values ('".$foliopago."', '".$Factura."','".$ImporteFac."','CONTADO','NACIONAL','".$Fech."','PAGO CON RECIBO DE FONDOS','-','".$Referencia."','".$Fecha."', '".$restante."','0', '0', '".$IdEmpresa."')";			
                    ...
                    if($a > 0){
						$QInsert = "Insert into cargosaplicaciones (foliopago, Factura, Importe, FP, Moneda, FechaV, TipoPago, TadBanco, Documento, Fecha, Abono, Intereses, IVA, IdEmpresa) 
						    values ('".$foliopago."', '".$Factura."','".$ImporteFac."','CONTADO','NACIONAL','".$Fech."','PAGO CON RECIBO DE FONDOS','-','".$Referencia."','".$Fecha."', '".$a."','0', '0', '".$IdEmpresa."')";	
						$result = mysqli_query($connect,$QInsert); 
						if(($result)==1){
                            $Importe = 0;
						} else {}
    ...
    function Contado(){ 		
        ...
        $Abono = $_POST["total"];         
                    ...
                    while($contador<20){
                        ...
                        if ($resultt->num_rows > 0) {
                            while($row = $resultt->fetch_assoc()) {  
                                ...
                                //break si no hay mas facturas a pagar
                    ...
                    if($sobra > 0 )
                    {
                        ...
                        values ('".$Fecha."','".$Concepto."', '".$Referencia."', '".$Abono."', '".$IDempresa."', '".$a."')";	
                    

                    }else{
			 	
						 $query = "Insert into movimientos (Fecha, Concepto, Referencia,  Abono, IdEmpresa, Disponible) 
                        values ('".$Fecha."','".$Concepto."', '".$Referencia."', '".$Abono."', '".$IDempresa."', '".$Abono."')";	
					...
					$result = mysqli_query($connect,$query);
						...
						if ($resulx->num_rows > 0) {
							...
							EnviarCorreo($Folio, $Correo, $Nombre, $Fecha, $Abono, $pdf, $NotaCredito);
						}
					} 
...
try {
    ...
    echo 'Pdf No Encontrado <br>';
}		
try {
	 
        $mail->AddAttachment($uploadfilexml);
} catch (Exception $e) {
    echo 'Xml No Encontrado <br>';
}		
...
Avenida Camaron Sábalo #102 local 12 col. Lomas de Mazatlán<br>
	        ...
	        ';                             //CIERRE DEL BODY (TEMPLATE ENVIAR CORREO)

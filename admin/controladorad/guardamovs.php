...

{
	...
	global $connect;
	 ...
	 if(isset($_POST['sel'])){ 
		...
		//(isset($_POST['sel'][$index]))==$_POST['idcondicion'][$index]
	...
	$Fecha = $_POST["sel"];
		...
		if((isset($_POST['sel'][$i]))==$F){

		//if(in_array($F, $_POST['sel'])){ 

		

		$RFC = "Select RzonSocial from empresa where idempresa = '".$_POST["IdEmpresa"][$i]."'";

			$rtf = $connect->query($RFC); 

					if ($rtf->num_rows > 0) {

					while($rocf = $rtf->fetch_assoc()) {  
			...
			echo "<script>alert('Empresa: ".$RS."  Importe: ".$F."');</script>";
	...
	if($Credito == 1){
		...
		$a = $_POST['Abono'][$i]; 
				...
				$query = "Insert into movimientos (Fecha, Concepto, Referencia,  Abono, IdEmpresa, Banco) 

			 values ('".$_POST['Fecha'][$i]."','".$_POST['Concepto'][$i]."', '".$_POST['Referencia'][$i]."', '".$_POST['Abono'][$i]."', '".$_POST['IdEmpresa'][$i]."', '".$_POST['Banco'][$i]."')";	
					...
					while($contador<20){
						...
						//get last factura where faltante >0 
							...
							$W = "Select folio, factura, fecha, restante, total from facturas where restante > 0 and idempresa = '".$_POST['IdEmpresa'][$i]."' order by fecha desc ";
					...
					$ImporteFac = $row['total']; 
						...
						if($a >= $restante){ 
							...
							values ('".$foliopago."','".$Factura."','".$ImporteFac."','CONTADO','NACIONAL','".$Fech."','PAGO CON RECIBO DE FONDOS','-','".$_POST['Referencia'][$i]."','".$_POST['Fecha'][$i]."', '".$restante."','0', '0', '".$_POST['IdEmpresa'][$i]."')";
							...
							values ('".$foliopago."','".$Factura."','".$ImporteFac."','CONTADO','NACIONAL','".$Fech."','PAGO CON RECIBO DE FONDOS','-','".$_POST['Referencia'][$i]."','".$_POST['Fecha'][$i]."', '".$a."','0', '0', '".$_POST['IdEmpresa'][$i]."')";
	...
	if($Credito == 0){
			...
			//echo $a;
				...
				$query = "Insert into movimientos (Fecha, Concepto, Referencia,  Abono, IdEmpresa, Disponible, Banco) 

			 values ('".$_POST['Fecha'][$i]."','".$_POST['Concepto'][$i]."', '".$_POST['Referencia'][$i]."', '".$_POST['Abono'][$i]."', '".$_POST['IdEmpresa'][$i]."', '".$_POST['Abono'][$i]."', '".$_POST['Banco'][$i]."')";	

			    	$result = mysqli_query($connect,$query);

					

				//	if($result->affected_rows > 0){  	echo "Guardado Correctamente";

...
if($_SERVER["REQUEST_METHOD"]=="POST"){
 ...
 for ($aaax = 0; $aaax < $num; $aaax++) {
	...
	echo '
		   ...
		   <input type="hidden" name="mensaje" value="Agregado Correctamentee">
	       </form>
		   </body> ';	 
	...
	function uno(){
		...
		$Fecha = $_POST["fecha"][$i]; 
		$NotaCredito = $_POST["notacredito"][$i];
		$pdf = $_POST["pdf"][$i]; 
			 ...
			 values ('".$Fecha."','".$Concepto."', '".$Referencia."', '".$Total."', '".$IdEmpresa."')";	
					...
					$ImporteFac = $row['total']; 
						...
						if($a >= $restante){ 
							...
							values ('".$foliopago."', '".$Factura."','".$ImporteFac."','CONTADO','NACIONAL','".$Fech."','PAGO CON RECIBO DE FONDOS','nx','".$Referencia."','".$Fecha."', '".$restante."','0', '0', '".$IdEmpresa."')";
							...
							if($a > 0){
							$QInsert = "Insert into cargosaplicaciones (foliopago, Factura, Importe, FP, Moneda, FechaV, TipoPago, TadBanco, Documento, Fecha, Abono, Intereses, IVA, IdEmpresa) 
							values ('".$foliopago."', '".$Factura."','".$ImporteFac."','CONTADO','NACIONAL','".$Fech."','PAGO CON RECIBO DE FONDOS','nx','".$Referencia."','".$Fecha."', '".$a."','0', '0', '".$IdEmpresa."')";

...
if (isset($_POST['idempresa'])) {
$sql = "SELECT IdEmpresa, Usuario, Activo, Contrasena, TipoUsuario, NoUsuario, Descripcion FROM usuarios where IdEmpresa = '".$_POST['idempresa']."'";
$id = $_POST['idempresa'];
} 
...
while($row = $result->fetch_assoc()) {
	$a = $row["TipoUsuario"]; 
	if($row["Activo"] == "1"){$activo = "Sí";}else{$activo = "No";}
	$TP = '';
	if($a == '1'){$TP = 'Administrador';}
	 if($a == '8'){$TP = 'Finanzas';}
	 if($a == '9'){$TP = 'Pedidos';}
	 if($a == '10'){$TP = 'Consultas';}
 $pass = substr_replace($row["Contrasena"], str_repeat('*', strlen($row["Contrasena"])), 0, strlen($row["Contrasena"]));
echo "<tr>
<form name='forma' method='post' action='controladorad/updateusuario.php'> 
...
<td> <label>".$row["Contrasena"]."</label></td> 
<td>  <select style='width:100%;' name='tipousuario'>
...
echo "
<tr>
<form name='formaagregar' method='post' action='controladorad/postusuario.php'> 
<input type='hidden' name='idempresa' value=".$id.">
<td height='24'><label>".$id."</label></td>
<td><label></label></td>
<td><input type='text' name='usuario' value =''  ></td>
<td><input type='text' name='contrasena' value =''    ></td>
<td> 
 <select style='width:100%;' name='tipousuario'> 

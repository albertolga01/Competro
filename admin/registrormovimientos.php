...
<!-- FIN DEL ENCABEZADO --> 
			 ...
			 <div class="margin">
				<DIV id=pathway>
							...
							<SPAN class=bullet>&nbsp;&nbsp;</SPAN> 
							 <a  href="menu_cteadmin" >Servicio a Clientes</a>&nbsp;<span class='bullet'>&nbsp;&nbsp;&nbsp;</span>&nbsp;<a  href="cargamovimientos" >Carga de movimientos</a>&nbsp;<span class='bullet'>&nbsp;&nbsp;&nbsp;</span>&nbsp;
	...
	<div id="maincontent" align="center"> 	
       ...
       <form name="forma1" method="post" action="registrormovimientos">
...
$stringg = date('Y-m-d', strtotime($string));
  $sql = "Select   Folio, IdEmpresa, Fecha, Concepto, Referencia, FechaCaptura, Abono from movimientos  where date(FechaCaptura) = '".$stringg."'";
...
<td> <div align='center' style='width:110px;'> <label>".$row["Concepto"]."</label> </div></td>
<td> <div align='center' style='width:110px;'> <label >".$row["Referencia"]."</label> </div></td>
<td> <div align='center' style='width:110px;'> <label >".$row["FechaCaptura"]."</label> </div></td>
<td> <div align='center' style='width:110px;'> <label >$".$importe."</label> </div></td>
...
<td class="color_blanco"></td>
<td class="color_blanco"></td>
<td class="color_blanco"> <div align="center" style="width:110px;">$'.number_format($TI, 2, '.', ',').'</div></td>
</tr>
...
die("Connection failed: " . $connect->connect_error);
  ...
  $sqld = "select date(fechacaptura) as fecha from movimientos where date(fechacaptura) >=  STR_TO_DATE('".$inicial."', '%d/%m/%Y') and date(fechacaptura) <= STR_TO_DATE('".$final."', '%d/%m/%Y') group by date(fechacaptura)";
...
$string = str_replace('/', '-', $Fecha); 
$stringg = date('Y-m-d', strtotime($string));
   $sql = "Select t1.Folio, t2.usuario, t1.Fecha, t1.Concepto, t1.Referencia, t1.FechaCaptura, t1.Abono as Importe from movimientos t1 inner join empresa t2 on t1.IdEmpresa = t2.IdEmpresa  where date(FechaCaptura) = '".$rowd['fecha']."'";
$cont = 0;
$TI = 0;
$result = $connect->query($sql);
if ($result->num_rows > 0) {

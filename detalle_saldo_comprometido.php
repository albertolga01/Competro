...
<!-- FIN DEL ENCABEZADO --> 
			 ...
			 <div class="margin">
				<DIV id=pathway>
							...
							<SPAN class=bullet>&nbsp;&nbsp;</SPAN> 
							 ...
							 <a class=bold href="saldo_analitico">Saldo Analítico</a>&nbsp;<span class='bullet'>&nbsp;&nbsp;&nbsp;</span>&nbsp;
   ...
   <table align="center" class="parametros">
			...
			$res = $connect->query("select usuario, t2.Credito, t2.LimC, IF(t2.Credito = '1', 'CREDITO', 'CONTADO') as FP, t2.SaldoCredito, sum(t3.Restante) as Restante from empresa t1 inner join estadocuenta t2 on t1.IdEmpresa = t2.IdEmpresa inner join facturas t3 on t2.IdEmpresa = t3.IdEmpresa where t1.idempresa= '".$_SESSION["idempresa"]."' ");
...
?>
			...
			<tr class=""><td align="left" class="color_blanco"><b>Saldo disponible:</b></td><td align="left" class="color_blanco">
<?php 
...
$result = $connect->query("SELECT folio, producto, destino, fecha, estadoatencion, comprometido FROM pedido WHERE idempresa= '".$_SESSION["idempresa"]."' AND comprometido > 0");
...
die("Connection failed: " . $conn->connect_error);
	...
	$sql = "SELECT folio, producto, destino, fecha, estadoatencion, comprometido FROM pedido WHERE idempresa= '".$IdEmpresa."' AND fecha = '".$rowd["fecha"]."' and comprometido > 0  and estadoatencion != 'Facturado' and estadoatencion != 'Cancelado' ";

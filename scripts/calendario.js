...
var fondoWeekend = "#00cd66";
var fondoVentana = "#2c5d3f";   
var fondoHeader = "#2e473b";
var fondoCalendar = "#70db93";
var fondoNomSemana = "#2e473b";
var borderColor1 = "003366";
var borderColor2 = "white";
var fondoSelMes = "#3d9140";
var colorTextoCal = "#003366";
var colorTextoHeader = "#97b4e1";
var colorNomSemana = "white";
var colorHoy = "white";
var colorDiaHoy = "white";
var colorWeekend = "#888888";
...
Calendar.SMonths = ["Ene", "Feb", "Mar", "Abr", "May", "Jun", "Jul", "Ago",
	"Sep", "Oct", "Nov", "Dic"];
// Month names in English
Calendar.EMonths = ["Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio",
	"Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"];
...
Calendar.DOW = ["Domingo", "Lunes", "Martes", "Miércoles", "Jueves", "Viernes", "Sábado"];
Calendar.count = 0;

// For inline calendar, the default contents of the layer. (v1.3)
Calendar.gInitText = "Softricks.com Calendario";
...
function Calendar(p_item, p_WinCal, p_month, p_year, p_format, p_type) {
	...
	if (this.gType == "INLINE") {
		this.WHO = "";
		this.INLINE = "Calendario";	// Inline Calendar Layer name
	...
	this.gTitle = "Softricks.com Calendario";
}
...
function Calendar_processDateData(vInputData, pFormat) {
	...
	if ((m = vInputData.match(ree)) != null) {
		if (m[1].toUpperCase() == "HOY" && m[0].length > 5)
			return dateAdd(td, m[4], m[3]);
		else if (m[1].toUpperCase() == "HOY")
			return td;
	} else {
...
function Calendar_readDate(indt, infmt) {
	...
	for (i=0; i<mach.length; i++) {
		switch (mach[i].toUpperCase()) {
			...
			case "DOW":
				re += "(LUNES|MARTES|MIERCOLES|JUEVES|VIERNES|SABADO|DOMINGO)";
				whoarr.push(mach[i]); break;
			case "MM":
				re += "(\\d{1,2})"; whoarr.push(mach[i]); break;
			case "MON":
				re += "(ENE|FEB|MAR|ABR|MAY|JUN|JUL|AGO|SEP|OCT|NOV|DIC)";
				whoarr.push(mach[i]); break;
			case "MONTH":
				re += "(ENERO|FEBRERO|MARZO|ABRIL|MAYP|JUNIO|JULIO|AGOSTO|SEPTIEMBRE|OCTUBRE|NOVIEMBRE|DICIEMBRE)";
				whoarr.push(mach[i]); break;
			case "YY":
				re += "(\\d{1,2})"; whoarr.push(mach[i]); break;
			case "YYYY":
...
Calendar.prototype.showSmartNavBar = function() {
	...
	var selcalendar = "<table border=0 bgcolor="+fondoSelMes+"><tr><td rowspan=2>";
	...
	selcalendar +=	"<\/select>\n</td>	<td>" + 
	   ...
	   ");\"><FONT FACE='" + fontface + "' SIZE=2 color="+colorTextoHeader+"><B><</B></FONT></A>" + 
	...
	selcalendar += "</td><td>" + 
		...
		"<FONT FACE='" + fontface + "' SIZE=2 color="+colorTextoHeader+"><B><</B></FONT></A>" + 
		"</td><TD rowspan=2 ALIGN=RIGHT> <FONT COLOR="+colorHoy+" FACE='" + fontface + "' SIZE=2><B>" + 
		"[<A HREF=\"" +"javascript:" + this.WHO + "Build(" + 
		"'" + this.gReturnItem + "', '" + gNow.getMonth() + "', '" + gNow.getFullYear() + "', '" + this.gFormat + "', '" + this.gType + "'" + 
		");\"><FONT color="+colorHoy+">Hoy</FONT></A>]</B></FONT></TD></tr><tr><td>" + 
		"<A HREF=\"" +	"javascript:" + this.WHO + "Build(" + 
	   "'" + this.gReturnItem + "', '" + nextMM + "', '" + nextYYYY + "', '" + this.gFormat + "', '" + this.gType + "'" + 
	   ");\"><FONT FACE='" + fontface + "' SIZE=2 color="+colorTextoHeader+"><B>></B></FONT></A></td><td>" + 
		"<A HREF=\"javascript:void(0);\" onClick=\"document.frmCal.cy.value++;document.frmCal.cy.onchange();return false;\">" + 
		"<FONT FACE='" + fontface + "' SIZE=2 color="+colorTextoHeader+"><B>></B></FONT></A>" + 
		"</td></tr></table>";
...
Calendar.prototype.show = function() {
	...
	this.wwrite("<link rel="icon" href="img/favicon.ico"> </head>");

	this.wwrite("<body " + 
			"bgcolor="+fondoVentana+" " + 
	...
	this.wwrite("<TABLE WIDTH='" + (v_Width-30) + "' BORDER=0 CELLPADDING=0 BGCOLOR="+borderColor1+"><TR><TD>" +  
		"<TABLE BORDER=0 WIDTH='100%' CELLPADDING=1 BGCOLOR="+borderColor2+">");  
	this.wwrite("<TR><TD BGCOLOR="+fondoHeader+" COLSPAN=2 >");
	this.wwriteA("<FONT COLOR=white FACE='" + fontface + "' SIZE=2><B>");
	this.wwriteA(this.gMonthName + " " + this.gYear);
	this.wwriteA("</B></TD>");
	

	this.wwriteA("</TR><TR><TD BGCOLOR="+fondoSelMes+" COLSPAN=2 ALIGN=CENTER>");
...
Calendar.prototype.showY = function() {
	...
	this.wwrite("<TABLE WIDTH='" + (v_Width-30) + "' BORDER=1 CELLSPACING=0 CELLPADDING=0 BGCOLOR="+borderColor2+"><TR><TD ALIGN=center>");
	this.wwrite("[<A HREF=\"" +
...
Calendar.prototype.cal_header = function() {
	...
	for (i=v_WeekStart,j=0; j<7; i++,j++,i=i%7)
		vCode = vCode + "<TD WIDTH='" + ((i==6) ? 16 : 14) + "%' ALIGN=CENTER BGCOLOR="+fondoNomSemana+">" + 
...
Calendar.prototype.cal_data = function() {
	...
	*/
	...
	for (j=v_WeekStart,i=0; j!=vFirstDay; j++,i++,j=j%7) {
		vCode = vCode + "<TD WIDTH='14%'" + this.write_weekend_string(j) + "><FONT SIZE='2' FACE='" + fontface + "' ALIGN=CENTER>&nbsp;</FONT></TD>";
	}
	...
	for (j=vFirstDay; i<7; j++,i++,j=j%7) {
		...
		vCode = vCode + "<TD WIDTH='14%'" + this.write_weekend_string(j) + 
				" ALIGN=CENTER><FONT SIZE='2' FACE='" + fontface + "'> <B>" + 
				linkText + 
	...
	for (m=1; m<(7-i); m++) {
		if (this.gYearly)
			vCode = vCode + "<TD WIDTH='14%'" + this.write_weekend_string(j+m) + 
				" ALIGN=CENTER><FONT SIZE='2' FACE='" + fontface + "' COLOR="+colorWeekend+">&nbsp;</FONT></TD>";
		else
			vCode = vCode + "<TD WIDTH='14%'" + this.write_weekend_string(j+m) + 
				" ALIGN=CENTER><FONT SIZE='2' FACE='" + fontface + "' COLOR="+colorWeekend+"> <B> " + m + "</B> </FONT></TD>";
	}
...
Calendar.prototype.format_day = function(vday) {
	...
	if (vday == vNowDay && this.gMonth == vNowMonth && this.gYear == vNowYear)
			return ("<FONT COLOR="+colorDiaHoy+" SIZE=3><B><I>" + vday + "</I></B></FONT>");
	else
			return (vday);
}
...
function Build(p_item, p_month, p_year, p_format, p_type, p_custom) {
	...
	if (p_type == "POPUP") {
		...
		if (v_PopupX >= 0 && v_PopupY >= 0) {
			if (!ggWinCal || ggWinCal.closed)
				ggWinCal = window.open("", "Calendario", 
	...
	if ( parseInt(p_year,10).toString() == "NaN" || parseInt(p_year,10)  < 1000 ){
		var vCurrentDate2;
		vCurrentDate2 = new Date();
		p_year = vCurrentDate2.getFullYear().toString();		
	}
	...
	gCal.gBGColor=fondoCalendar;
	gCal.gLinkColor=colorTextoCal;
	gCal.gTextColor=colorTextoCal;
	gCal.gHeaderColor=colorNomSemana;
	// ############ CUSTOMIZE #############
...
//AppendOrReplace=Replace;AppendChar=';';CloseOnSelect=Yes;ReturnData=Date;Title=Calendar;InlineX=400;InlineY=50;CurrentDate=09-Aug-1969;SelectAfter=Today-30;SelectBefore=Today+30;AllowWeekends=No;Resizable=Yes;CallFunction=TryCallFunction;PopupX=300;PopupY=300;Nav=Yes;SmartNav=Yes;Fix=Yes;WeekStart=1;Weekends=16;

	if (arguments[5] == null)
		p_custom = "CloseOnSelect=Yes;AppendOrReplace=Replace;AppendChar=';';ReturnData=Date;Title=Popup Date Picker;AllowWeekends=Yes;Resizable=No;SmartNav=Yes;";
	else
		p_custom = typeof(arguments[5]) == "string" ?
					arguments[5]
					:
					"CloseOnSelect=Yes;AppendOrReplace=Replace;AppendChar=';';ReturnData=Date;Title=Popup Date Picker;AllowWeekends=Yes;Resizable=No;SmartNav=Yes;";
...
function show_yearly_calendar(p_item, p_year, p_format) {
	...
	var ggWinCal = window.open("", "Calendario", "scrollbars=yes");
	ggWinCal.opener = self;
